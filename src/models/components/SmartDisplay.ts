...
export class SmartDisplay extends Component {
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.fillText(
      `Mode: ${this.displayMode.toUpperCase()}`,
  ...
  onClick(point: Point): void {
    ...
    if (
      Math.sqrt(Math.pow(point.x - buttonX, 2) + Math.pow(point.y - buttonY, 2)) <= buttonRadius
    ) {
    ...
    for (let i = 0; i < this.bitWidth; i++) {
      ...
      if (
        ...
        point.x <= this.position.x + this.size.width - 40
      ) {
