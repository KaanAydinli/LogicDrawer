...
export class VerilogParser {
  ...
  private extractIfStatementsManually(
    alwaysBody: string,
    gates: VerilogGate[],
    gateCounter: number
  ): void {
    ...
    if (thenAssignMatch && elseAssignMatch && thenAssignMatch[1] === elseAssignMatch[1]) {
      ...
      if (
        this.isSimpleIdentifier(controlSignal) &&
        this.isSimpleIdentifier(trueSignal) &&
        this.isSimpleIdentifier(falseSignal)
      ) {
        const muxGate: VerilogGate = {
          type: "mux2",
        ...
        console.warn(
          `Complex expressions inside if/else branches not fully implemented for MUX generation yet: ${condition}`
        );
      ...
      console.warn(
        `Handling 'if' without 'else' for target '${thenAssignMatch[1]}' is not fully implemented (potential latch).`
      );
      
    } else {
      
      console.warn(
        `Assignments in 'if'/'else' branches do not match or are missing for condition: ${condition}`
      );
    }
  }
