...
export class TruthTableImageTool implements Tool {
  async execute(context: ToolContext): Promise<string> {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/api/generate/gemini-vision`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: "Extract the truth table from this image. Return only a JSON object with format: {headers: [column names], rows: [[values in row 1], [values in row 2], ...]}. Only include the actual table data, no explanations.",
          imageData: context.image
        }),
      });
  ...
  private parseTableText(text: string): { headers: string[], rows: string[][] } {
    ...
    for (let i = 1; i < lines.length; i++) {
      const row = lines[i].split(delimiter).map(cell => cell.trim()).filter(cell => cell);
      if (row.length > 0) {
        rows.push(row);
      }
    }
...
export class KMapImageTool implements Tool {
  async execute(context: ToolContext): Promise<string> {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/api/generate/gemini-vision`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: "Extract the Karnaugh map from this image. Return only a JSON object with format: {variables: [list of variables], rows: number of rows, cols: number of columns, values: [[row 1 values], [row 2 values], ...]}. Only include the actual K-map data, no explanations.",
          imageData: context.image
        }),
      });
      ...
      const { truthTable, inputLabels, outputLabels } = this.convertKMapToTruthTable(extractedKMapData);
      
      // Create circuit from the K-Map data
      const success = await this.createCircuitFromKMap(truthTable, inputLabels, outputLabels, context.circuitBoard);
  ...
  private parseKMapText(text: string): { variables: string[], rows: number, cols: number, values: number[][] } {
    const lines = text.split('\n').filter(line => line.trim().length > 0);
  ...
  private convertKMapToTruthTable(kmap: { variables: string[], rows: number, cols: number, values: number[][] }): {
    truthTable: { inputs: boolean[], outputs: boolean[] }[],
    inputLabels: string[],
    outputLabels: string[]
  } {
    const inputLabels = kmap.variables || ['A', 'B', 'C', 'D'].slice(0, Math.log2(kmap.rows * kmap.cols));
  ...
  private getGrayCodeMapping(size: number): number[][] {
    ...
    if (size === 4) return [[0,0], [0,1], [1,1], [1,0]];
