...
export class CircuitSuggester {
  ...
  private loadPatterns(): void {
    ...
    Logger.log(`Loaded ${this.patterns.length} circuit patterns`);
  }
  ...
  private analyzeCircuit(): void {
    ...
    if (bestMatch) {
      Logger.log(
        `Found matching pattern: ${bestMatch.pattern.name} (${Math.round(bestMatch.similarity * 100)}% similarity)`
      );
  ...
  private calculateSimilarity(currentCircuit: string, patternCircuit: any): number {
    const current =
      typeof currentCircuit === "string" ? JSON.parse(currentCircuit) : currentCircuit;
    const pattern =
      typeof patternCircuit === "string" ? JSON.parse(patternCircuit) : patternCircuit;
    ...
    const wireSimilarity =
      patternWires.length > 0 ? Math.min(1, currentWires.length / patternWires.length) : 0;
  ...
  private createGhostCircuit(pattern: CircuitPattern): void {
    ...
    const patternCircuit =
      typeof pattern.data === "string" ? JSON.parse(pattern.data) : pattern.data;
    ...
    Logger.log(`Created ghost circuit with ${this.ghostComponents.length} components`);
  }
  ...
  private acceptSuggestion(pattern: CircuitPattern): void {
    Logger.log(`Accepting suggestion: ${pattern.name}`);
    const patternCircuit =
      typeof pattern.data === "string" ? JSON.parse(pattern.data) : pattern.data;
  ...
  private importPatternToCircuit(patternData: any): void {
    ...
    for (const compData of patternData.components) {
      if (
        currentComponentsByType[compData.type] &&
        currentComponentsByType[compData.type].length > 0
      ) {
